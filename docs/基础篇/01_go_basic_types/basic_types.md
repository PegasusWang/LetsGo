...
给一个 go 变量命名(标识符)的时候，通常使用大骆驼(BigCamel)和小骆驼(smallCamel)命名法，只要保证不和内置的25个关键字冲突就行。
...
![go 预定义标识符](./builtin_identifier.png)
...
go 中声明一个类型很简单，使用 var 关键字就可以了，声明之后默认使用其类型的零值初始化。
